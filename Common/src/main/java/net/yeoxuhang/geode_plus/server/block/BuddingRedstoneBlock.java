...
public class BuddingRedstoneBlock extends RedStoneOreBlock {
    ...
    public void randomTick(BlockState p_220898_, ServerLevel p_220899_, BlockPos p_220900_, RandomSource p_220901_) {
        if (p_220901_.nextInt(5) == 0) {
            ...
            } else if (blockstate.is(BlockRegistry.SMALL_REDSTONE_BUD.value()) && blockstate.getValue(
                    DropExperienceClusterBlock.FACING) == direction) {
                block = BlockRegistry.MEDIUM_REDSTONE_BUD.value();
            } else if (blockstate.is(BlockRegistry.MEDIUM_REDSTONE_BUD.value()) && blockstate.getValue(
                    DropExperienceClusterBlock.FACING) == direction) {
                block = BlockRegistry.LARGE_REDSTONE_BUD.value();
            } else if (blockstate.is(BlockRegistry.LARGE_REDSTONE_BUD.value()) && blockstate.getValue(
                    DropExperienceClusterBlock.FACING) == direction) {
