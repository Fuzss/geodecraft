...
public class BuddingRedstoneBlock extends RedStoneOreBlock {
    ...
    public void randomTick(BlockState p_220898_, ServerLevel p_220899_, BlockPos p_220900_, RandomSource p_220901_) {
        if (p_220901_.nextInt(5) == 0) {
            ...
            if (canClusterGrowAtState(blockstate)) {
                block = BlockRegistry.SMALL_REDSTONE_BUD.value();
            } else if (blockstate.is(BlockRegistry.SMALL_REDSTONE_BUD.value()) && blockstate.getValue(OresClusterBlock.FACING) == direction) {
                block = BlockRegistry.MEDIUM_REDSTONE_BUD.value();
            } else if (blockstate.is(BlockRegistry.MEDIUM_REDSTONE_BUD.value()) && blockstate.getValue(OresClusterBlock.FACING) == direction) {
                block = BlockRegistry.LARGE_REDSTONE_BUD.value();
            } else if (blockstate.is(BlockRegistry.LARGE_REDSTONE_BUD.value()) && blockstate.getValue(OresClusterBlock.FACING) == direction) {
                block = BlockRegistry.REDSTONE_CRYSTAL.value();
            }
            if (block != null) {
    ...
    public List<ItemStack> getDrops(BlockState blockState, LootParams.Builder builder) {
        ...
        if (GeodePlus.hasSilkTouch(builder, pickaxe) && ServerConfigs.BLOCKS.allowSilkTouch && blockState.is(BlockRegistry.BUDDING_REDSTONE.value())){
            return Collections.singletonList(stone);
        }
        if (GeodePlus.hasSilkTouch(builder, pickaxe) && ServerConfigs.BLOCKS.allowSilkTouch && blockState.is(BlockRegistry.BUDDING_DEEPSLATE_REDSTONE.value())){
            return Collections.singletonList(deepslate);
        }
        if (GeodePlus.hasSilkTouch(builder, pickaxe) && ServerConfigs.BLOCKS.allowSilkTouch && blockState.is(BlockRegistry.BUDDING_SCULK_REDSTONE.value())){
